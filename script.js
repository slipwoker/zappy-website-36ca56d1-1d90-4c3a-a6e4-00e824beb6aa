document.addEventListener('DOMContentLoaded', function () {
  ...
  document.body.addEventListener('click', function (e) {
    ...
    const top =
      destination.getBoundingClientRect().top + window.pageYOffset - offset;
  ...
  if (contactForm) {
    ...
    function getErrorMessage(field, rule) {
      ...
      const messages = {
        ...
        minlength: label + ' must be at least ' + field.getAttribute('minlength') + ' characters.'
      };
    ...
    function showError(field, message) {
      ...
      errorEl.id = 'error-' + (field.id || field.name || Math.random().toString(36).slice(2));
    ...
    function validateField(field) {
      ...
      if (field.hasAttribute('minlength') && !VALIDATORS.minlength(value, field.getAttribute('minlength'))) {
    ...
    contactForm.addEventListener('input', function (e) {
      const field = e.target;
      if (field.matches('input, textarea, select') && field.classList.contains('input-error')) {
        validateField(field);
      }
    });
  }
  ...
  const OBSERVE_CLASS = 'fade-in-element';
  ...
  function initScrollAnimations() {
    ...
    elements.forEach(function (el, index) {
      el.classList.add(OBSERVE_CLASS);

      if (!el.style.transitionDelay) {
        const siblings = Array.from(el.parentNode.children).filter(function (child) {
          return child.classList.contains(OBSERVE_CLASS);
        });
    ...
    if (!document.getElementById('fade-in-styles')) {
      ...
      style.textContent = [
        '.' + OBSERVE_CLASS + '{opacity:0;transform:translateY(24px);transition:opacity 0.6s ease,transform 0.6s ease;}',
        '.' + OBSERVE_CLASS + '.' + ANIMATION_CLASS + '{opacity:1;transform:translateY(0);}'
      ].join('');
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Gpl משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '36ca56d1-1d90-4c3a-a6e4-00e824beb6aa',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '36ca56d1-1d90-4c3a-a6e4-00e824beb6aa';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '36ca56d1-1d90-4c3a-a6e4-00e824beb6aa';
